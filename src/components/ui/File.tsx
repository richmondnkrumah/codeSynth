...
const getIconSVG = (fileName: string):ReactElement => {
  ...
  const language: string = tempFileName.length >= 2 ? tempFileName.slice(-1).toString() : 'default'
...
const File = ({name}: props) => {
  ...
  const language: string = tempName.length >= 2 ? tempName.slice(-1).toString() : ''
  const iconSvg = getIconSVG(name)  
  const handleFileClick = () => {
    addNewFile({fileName: name,language,content: "",icon: iconSvg})
  }
  // const getIcon = (): number => {
  //   const foundIcon = ICONS.findIndex(icon => icon.name === language)
  //   return foundIcon;
  // }
